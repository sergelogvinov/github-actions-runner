...
var gracefulShutdown = function () {
  ...
  stopping = true;
  if (listener) {
    fs.writeFile('/tmp/shutdown', '', function (err) {
      if (err) throw err;
      console.log('Sending graceful shutdown to runner listener');
    });
    ...
    var timer = setInterval(() => {
      if (!fs.existsSync('/tmp/shutdown')) {
        console.log('Runner listener exited');
        clearInterval(timer);
        cleanUp();
      }

      runCount++;
      if (runCount > 6 * 60) { // 60 minutes
        clearInterval(timer);
        cleanUp();
      }
    }, 10000);
